...
use bitcoincore_rpc::bitcoin::{hex, Address, Amount, BlockHash, Network, PublicKey, ScriptBuf, Transaction, Txid};
...
fn send(rpc: &Client, addr: &str, amt: Amount, txid: &str, vout: u32) -> bitcoincore_rpc::Result<String> {
    let args = [
...
fn load_or_create_wallet(name: &str, rpc: &Client) -> bitcoincore_rpc::Result<LoadWalletResult> {
    ...
    match wallet {
        ...
        Err(bitcoincore_rpc::Error::JsonRpc(e)) if e.to_string().contains("Path does not exist") => {
...
fn main() -> bitcoincore_rpc::Result<()> {
    ...
    let miner_address = miner_wallet_rpc.get_new_address(None, None)?.assume_checked();
    ...
    let trader_address = trader_wallet_rpc.get_new_address(None, None)?.assume_checked();
    // println!("trader_address: {trader_address}");

    // Send 20 BTC from Miner to Trader
    let txhash = send(&miner_wallet_rpc, &trader_address.to_string(), Amount::from_int_btc(20), &viable.txid.to_string(), viable.vout)?;
    ...
    let confirmed_tx = block.txdata.iter().find(|tx| tx.txid() == txid_transfer).unwrap();
    ...
    let trader_out = confirmed_tx.output.iter().find(|o| is_mine(&trader_wallet_rpc, &o.script_pubkey)).unwrap();
    ...
    let miner_change = confirmed_tx.output.iter().find(|o| is_mine(&miner_wallet_rpc, &o.script_pubkey)).unwrap();
